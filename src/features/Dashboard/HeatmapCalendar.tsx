// src/features/Dashboard/HeatmapCalendar.tsx
...
const CalendarHeatmap: React.FC<Props> = ({ data, startDate, isCurrentChunk }) => {
...
const windowEnd = addDays(parsed, WEEKS * DAYS - 1);
const now = new Date();
// const isCurrentChunk = format(now, "yyyy-MM-dd") <= format(windowEnd, "yyyy-MM-dd");
...
const val = (isCurrentChunk && isAfter(d, now)) ? null : lookup.get(iso);
